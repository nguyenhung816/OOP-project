...
public class MatchCards {
    ...
    void checkIfLevelCompleted() { 
        ...
        if (level < maxLevel) {
            ...
            String[] options = {"Restart from Level 1", "Return to Main Menu"};
            int choice = JOptionPane.showOptionDialog(frame, "Would you like to restart from Level 1 or return to the main menu?", 
                    "Game CompletedCompleted", JOptionPane.YES_NO_OPTION, JOptionPane.INFORMATION_MESSAGE, null, options, options[0]);

            if (choice == JOptionPane.YES_OPTION) {
                level = 1;
                lives = 5; 
                retries = 0; 
                setupNewLevel(); //setup new level
            } else {
